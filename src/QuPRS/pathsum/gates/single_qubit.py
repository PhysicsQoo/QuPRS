...
def h(self: "PathSum", qubit: int | str, is_bra: bool = False) -> "PathSum":
    ...
    if not is_bra:
        ...
        x_i = se.symbols(
            f"{self.bits[qubit]}" if isinstance(qubit, int) else str(qubit)
        )
...
def y(self: "PathSum", qubit: int | str | se.Symbol, is_bra: bool = False) -> "PathSum":
    if not is_bra:
        ...
        new_P = (
            self.P.subs(x_i, 1 - x_i) + se.Rational(3, 4) + se.Rational(1, 2) * x_i
        ).expand()
        new_f = self.f.sub(x_i, sp.Not(x_i))
    new_P = reduce_expression(new_P)
    return PathSum(new_P, new_f, self.pathvar)
...
@gate(type="single")
def sdg(
    self: "PathSum", qubit: int | str | se.Symbol, is_bra: bool = False
) -> "PathSum":
    if not is_bra:
        x_i = logical_to_algebraic(self.f[qubit], 2)
        new_P = (self.P + se.Rational(-1, 4) * x_i).expand()
...
@gate(type="single")
def p(
    self: "PathSum", theta, qubit: int | str | se.Symbol, is_bra: bool = False
) -> "PathSum":
    theta = div_pi(theta)
    if not is_bra:
        if theta.is_number:
...
@gate(type="single")
def u(
    self: "PathSum", theta, phi, lam, qubit: int | str | se.Symbol, is_bra: bool = False
) -> "PathSum":
...
@gate(type="single")
def u2(
    self: "PathSum", phi, lam, qubit: int | str | se.Symbol, is_bra: bool = False
) -> "PathSum":
