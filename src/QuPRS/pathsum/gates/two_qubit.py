...
def cx(
    ...
    if not is_bra:
        new_f = self.f.update(
            target_qubit, to_anf(sp.Xor(self.f[control_qubit], self.f[target_qubit]))
        )
...
@gate(type="two")
def ch(
    self: "PathSum", control_qubit: int, target_qubit: int, is_bra: bool = False
) -> "PathSum":
...
@gate(type="two")
def CRk(
    self: "PathSum", k: int, control_qubit: int, target_qubit: int, is_bra: bool = False
) -> "PathSum":
    if not is_bra:
...
@gate(type="two")
def crz(
    self: "PathSum", theta, control_qubit: int, target_qubit: int, is_bra: bool = False
) -> "PathSum":
    theta = div_pi(theta)
    if not is_bra:
        if theta.is_number:
...
def cu3(
    ...
    new_pathsum = new_pathsum.cp(
        se.Rational(1, 2) * (phi + lam), control_qubit, target_qubit, is_bra
    )  # global phase
    return new_pathsum
...
def swap(
    ...
    if not is_bra:
        ...
        x_i = se.symbols(
            f"{self.bits[qubit1]}" if isinstance(qubit1, int) else str(qubit1)
        )
        x_j = se.symbols(
            f"{self.bits[qubit2]}" if isinstance(qubit2, int) else str(qubit2)
        )
